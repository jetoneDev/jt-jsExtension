...
class SqlTranslator {
    ...
    //insert
    insert(sql, params,contrastData,extraDatalist) {
        let result = '';
        ...
        params.forEach((element, index) => {
            ...
            for (let i = 0; i < sqlParamArray.length; i++) {
                ...
                // 数据是否在对照表里面
                else if(contrastData.aliasList[sqlParamArray[i]]){
                    valueArray.push(element[contrastData.aliasList[sqlParamArray[i]]]);
                }
                // 数据源没有的数据(如创建人,创建人Id)
                else{
                    valueArray.push(contrastData.extraDatalist[sqlParamArray[i]]);
                }
            }
